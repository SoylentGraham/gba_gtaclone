...
//	reset_tilemap(0,0,road_map,building_map, shadow_map, 3, 3, 2 );
	...
	init_sprite( &sprites[0], SIZE_8, FONT_TILES, 0, 0, 0);
	//move_sprite( &sprites[0], 10, 10);
	rotate_sprite(&sprites[0], 270,ZOOM(1),ZOOM(1));
	make_sprite_anim( &sprite_animations[0], &sprites[0], OBJ_TILE(FONT_TILES,0), tile_set_lengths[FONT_TILES]-1, true, 2);
...
/*
		if (old_selected_option != selected_option)
			draw_string(1,MENU_TOP+old_selected_option,false," ");
		
		draw_string(1,MENU_TOP+selected_option,false,"Z");
*/
