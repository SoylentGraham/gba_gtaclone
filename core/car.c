...
void init_car( a_car *car, a_sprite *sprite, a_car_type *car_type , s16 init_tile_x, s16 init_tile_y, u16 init_ang, u8 collide_data, u8 ai_mode)
{
	...
	init_sprite( car->sprite, SIZE_16, CAR_TILES, 0, 3, car->rot_id);
...
//working: steering_ang contains change in driving ang. 
//always decrease this in this func when being used
void re_align_steering( a_sprite *sprite)
{
	if ( sprite->real_speed==0 ) return;

	//stuff to do:
		//add steering input to current ang
		//if no input move steering_ang back towards 0 <-- based on speed
		//if steering_ang near 0, and speed, move driving ang towards a n*45 degree angle

	//store current steering direction
	sprite->last_steer_dir = sprite->steer_dir;

	//calc a steering change
	s16 steer_amount = 0;
	if ( sprite->steer_dir != STEER_NONE )
		//steer_amount = sprite->real_speed / 4; //or something 
		steer_amount = 1; //or something 


	//sudden(?) change in direction
	if ( ( sprite->steer_dir == STEER_LEFT 
		&& sprite->last_steer_dir == STEER_RIGHT ) 
		|| ( sprite->steer_dir == STEER_RIGHT 
		&& sprite->last_steer_dir == STEER_LEFT ) )
	{
		//skid/screech if at a decent speed
	}


	//if steering, change our steering direction
	if ( sprite->steer_dir != STEER_NONE )
	{
		if ( sprite->steer_dir == STEER_LEFT && (sprite->steering_ang>>8) > -MAX_STEER )
		{
			sprite->steering_ang -= steer_amount ;
			if ( (sprite->steering_ang >>8) < -MAX_STEER )
				sprite->steering_ang = (-MAX_STEER) <<8 ;
		}

		if ( sprite->steer_dir == STEER_RIGHT && (sprite->steering_ang>>8) < MAX_STEER )
		{
			sprite->steering_ang += steer_amount ;
			if ( (sprite->steering_ang >>8) > MAX_STEER )
				sprite->steering_ang = (MAX_STEER) <<8 ;
	...
	//straighten up (to near 45) if no input to change dir
	if ( sprite->steer_dir == STEER_NONE )
	{
		//get nearest 45o ang, then set steering ang to difference/factor and base on speed too
		//s16 nearest_45_degree=nearest_45_ang( sprite->driving_ang>>8 );
	}


	//steering defficeny amount
	byte de_steer_rate=DE_STEER; //modify by speed


	//modify driving angle
	sprite->driving_ang += sprite->steering_ang;


	//decrease the steering angle 
//	if ( abs(sprite->steering_ang) > de_steer_rate )
//		sprite->steering_ang += sprite->steering_ang/de_steer_rate;
//	else
//	{
//		if ( sprite->steering_ang > 0 )
//			sprite->steering_ang -= 1;
//		else
//			sprite->steering_ang += 1;
//	}
	...
	if ( sprite->steering_ang > 0 )
		sprite->steering_ang -= 1;
	else if ( sprite->steering_ang < 0 )
		sprite->steering_ang += 1;
	


	//modify this if skidding/handbraking, hard braking turns etc
	sprite->travel_ang = sprite->driving_ang;
}
